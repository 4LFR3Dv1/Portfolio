...
import type { ArchitectureView, ArchitectureViewId } from './architecture';

export type PublicArchitectureViewId = ArchitectureViewId;

type Localized<T> = Record<Language, T>;
const localized = <T>(en: T, pt: T): Localized<T> => ({ en, pt });

export const publicArchitectureViews: ArchitectureView[] = [
  {
    id: 'systems',
    index: '00',
...
export function getPublicArchitectureView(value: string | null | undefined): ArchitectureView {
  return publicArchitectureViews.find((view) => view.id === value) ?? publicArchitectureViews[0];
}
